...
from DatabaseUtil.Mysql import *
...
class JqdataClient:
    ...
    def init(self, username: str = "", password: str = "") -> bool:
        ...
        try:
            ...
            jq.auth("17694842438", "Zhangshaopu123.")
    ...
    def query_history_from_mysql(self, req: HistoryRequest) -> Optional[List[BarData]]:
        # if self.symbols is None:
        #     return None
        ...
        exchange = req.exchange
        interval = req.interval
        start = req.start # '2022-07-06'
        end = req.end

        mysql = Mysql()
        sql = "SELECT DATE,TIME,OPEN,HIGH,LOW,CLOSE FROM xauusd_data WHERE DATE BETWEEN %s AND %s "
        result = mysql.getAll(sql,(start , end))
        print("查询成功")
        mysql.dispose()
        ...
        if result is not None:
            for rowdata in result:
                date_str = rowdata['DATE']
                time_str = rowdata['TIME'] 
                ...
                bar = BarData(
                    ...
                    interval=interval,
                    datetime=dt,
                    open_price=rowdata["OPEN"],
                    high_price=rowdata["HIGH"],
                    low_price=rowdata["LOW"],
